                ...
                if motif == "CG" {
                    if r.is_reverse() {
                        if pos_usize as i32 - start as i32 - 1 > 0 {
                            methyl_pos_dict.insert(pos_usize - 1 - start, qual);
                        }
                    } else {
                        methyl_pos_dict.insert(pos_usize - start, qual);
                    }
                } else {
...
pub fn aggregate_methylation_reads(
    methyl_all_reads: HashMap<String, HashMap<usize, f32>>,
    min_methyl_threshold: f32,
) -> HashMap<usize, f32> {
    ...
    for (pos, qual_list) in aggregated_methyl_pos_dict.iter() {
        ...
        let mod_score = if hyper_methyl + hypo_methyl == 0 {
            0.0
        } else {
            hyper_methyl as f32 / (hyper_methyl + hypo_methyl) as f32
        };
...
pub fn start(
    bam_records: HashMap<String, Record>,
