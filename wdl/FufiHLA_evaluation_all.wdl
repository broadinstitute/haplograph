...
workflow Evaluation {
    ...
    scatter (desiredCoverage in desiredCoverages) {
        ...
        call FufiHLA.FuFiHLA as Fufihla {
            input:
                whole_genome_bam = downsampleBam.downsampled_bam,
                whole_genome_bai = downsampleBam.downsampled_bai,
                prefix = prefix + "_" + desiredCoverage,
                gene_bed = gene_bed
        }
        Map[String, Array[File]] fufihla_outputs = {
            "HLA-A":   Fufihla.fufihla_output.HLA_A,
            "HLA-B":   Fufihla.fufihla_output.HLA_B,
            "HLA-C":   Fufihla.fufihla_output.HLA_C,
            "HLA-DQA1": Fufihla.fufihla_output.HLA_DQA1,
            "HLA-DQB1": Fufihla.fufihla_output.HLA_DQB1,
            "HLA-DRB1": Fufihla.fufihla_output.HLA_DRB1
        }
        ...
        scatter (pair in zip(parseBed.locuslist, parseBed.genelist)) {
            ...
            call haplograph_eval {
                input:
                    ...
                    query_fasta = fufihla_outputs[gene_name],
    ...
    output {
        Array[Array[File]] hla_eval_result = haplograph_eval.qv_scores
    }
}
...
task haplograph_eval {
    ...
    command <<<
        set -euxo pipefail
        cat ~{sep=' ' query_fasta} > query.fasta
...
task CalculateCoverage {
    ...
    command <<<
        ...
        samtools view -bhX ~{bam} ~{bai} -L ~{bed_file} > ~{prefix}.bam
