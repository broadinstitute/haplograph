...
task PreProcessFile {
    input {
        ...
        Int disk_size
        Int minimal_case_num = 20
        Float minimal_af = 0.01
        Int allele_resolution = 1
    ...
    command <<<
        ...
        def main():
            ...
            # load genotypes
            df_genotype_ = pd.read_csv(args.genotype, index_col = 0)
            df_genotype_.index = df_genotype_.index.astype(str)

            D = {}
            for pid, row in df_genotype_.iterrows():
                for col, v in row.items():
                    if col.startswith("KIR"):
                        if args.resolution == 0:
                            allele = col.split("*")[0]
                        elif args.resolution == 1:
                            genebody = col.split("*")[0]
                            a = col.split("*")[1][:3]
                            allele = genebody + "*" + a                
                        ...
                        allele = ":".join(col.split(":")[:args.resolution])
                    D[pid] = D.get(pid, {})
                    D[pid][allele] = D[pid].get(allele, 0) + v
            df_genotype_all = pd.DataFrame(D).T.fillna(0)
